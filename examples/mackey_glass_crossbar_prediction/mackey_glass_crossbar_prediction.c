...
void generate_mackey_glass(double *buffer, size_t length, double x0, double tau,
			   double beta, double gamma, int n)
{
	...
	for (size_t i = 0; i < length - 1; i++) {
		...
		double k1 = mg_dt * (beta * x_tau / (1.0 + pow(x_tau, n)) -
				     gamma * x_t);

		// k2: slope at the midpoint, using k1
		x_tau = history[(i + history_len / 2) %
				history_len]; // Approx. delay for midpoint
		double k2 = mg_dt * (beta * x_tau / (1.0 + pow(x_tau, n)) -
				     gamma * (x_t + 0.5 * k1));

		// k3: slope at the midpoint, using k2
		double k3 = mg_dt * (beta * x_tau / (1.0 + pow(x_tau, n)) -
				     gamma * (x_t + 0.5 * k2));

		// k4: slope at the end of the interval, using k3
		x_tau = history[(i + 1) %
				history_len]; // Approx. delay for endpoint
		double k4 = mg_dt * (beta * x_tau / (1.0 + pow(x_tau, n)) -
				     gamma * (x_t + k3));

		// Update the current value using the weighted average of the
		// slopes
		x_t += (k1 + 2.0 * k2 + 2.0 * k3 + k4) / 6.0;

		// Store the new value in the history buffer (for future delayed
		// lookups)
...
int main(void)
{
	...
	if (output_file == NULL) {
		fprintf(stderr,
			"Error: Could not open data files for writing.\n");
		return 1;
	}
	...
	if (input_series == NULL) {
		fprintf(stderr,
			"Error: Failed to allocate memory for input_series.\n");
		fclose(output_file);
		return 1;
	}
	...
	spires_reservoir_config base = {
	    ...
	    .neuron_params =
		fractional_neuron_params, /* alpha will be set internally by
					     optimizer */
	...
	if (spires_train_ridge(R, input_series, target_series, T, lambda) !=
	    SPIRES_OK) {
	...
	spires_crossbar_readout_config crossbar_config = {
	    .num_neurons = base.num_neurons,
	    .num_outputs = base.num_outputs,
	    .num_timesteps = series_length,
	    .time_step = 1.0e-6,
	    .row_voltage_scaler = 0.01,
	    .load_resistance = 50.0,
	    .r_on = 1000.0,
	    .r_off = 100000.0,
	    .model = SPIRES_CROSSBAR_MODEL_YAKOPCIC,
	};

	double *crossbar_outputs = spires_run_crossbar_readout(
	    &crossbar_config, R, input_series, series_length);
	if (!crossbar_outputs) {
		fprintf(stderr, "crossbar inference failed\n");
		spires_reservoir_destroy(R);
		free(input_series);
		fclose(output_file);
		return 1;
	...
	fprintf(output_file, "# Timestep Input_Signal Crossbar_Output\n");
	for (size_t i = 0; i < series_length; i++) {
		fprintf(output_file, "%zu %f %f\n", i, input_series[i],
			crossbar_outputs[i]);
	}

	free(crossbar_outputs);
	spires_reservoir_destroy(R);
	fclose(output_file);
	free(input_series);
	return 0;
