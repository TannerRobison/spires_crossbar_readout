 ...
 *                                        alpha=1. O(mem_len) per neuron per
 * step. SPIRES_SYNAPSE_FRACTIONAL_MULTIEXP:  params[0] = alpha (target order),
...
typedef enum { SPIRES_EI_PER_SYNAPSE = 0, SPIRES_EI_PER_NEURON } spires_ei_mode;
...
typedef struct {
	...
	spires_synapse_type
	    synapse_type; /* default 0 = SPIRES_SYNAPSE_SIMPLE */
	spires_synapse_backend
	    synapse_backend;	/* default 0 = SPIRES_SYNAPSE_SPARSE */
	spires_ei_mode ei_mode; /* default 0 = SPIRES_EI_PER_SYNAPSE */
	spires_plasticity_type
	    plasticity_type;	   /* default 0 = SPIRES_PLASTICITY_NONE */
	double *plasticity_params; /* layout documented above; caller owns */
	double *synapse_params; /* forwarded as-is; layout documented above per
				   synapse_type; caller owns */
	 ...
	 *
	 * 0 is an ordinary seed, not a sentinel: a zero-initialised config
	 * gives a reproducible network rather than a different one each run.
	 * Pass spires_random_seed() to opt in to per-run variation. */
	uint64_t seed;
 ...
 * Pass NULL for zeros. No output buffer here; use state or your own output
 * accessor.
 */
 ...
 * input_series: flattened [series_length x num_inputs] (for Din=1 just
 * length=series_length) Returns a newly malloc'd array of series_length *
 * num_outputs doubles. Caller must free().
 */

double *spires_run(spires_reservoir *r, const double *input_series,
		   size_t series_length);

/* Run the reservoir on a series of inputs.
 * input_series: flattened [series_length x num_inputs] (for Din=1 just
 * length=series_length) output_buffer must hold series_length * num_outputs
 * doubles; the size cannot be checked here, so a buffer sized for one output
 * per step overflows when num_outputs > 1.
 */
...
typedef enum {
	SPIRES_CROSSBAR_MODEL_YAKOPCIC,
	SPIRES_CROSSBAR_MODEL_PERSHIN,
	SPIRES_CROSSBAR_MODEL_HP,
	SPIRES_CROSSBAR_MODEL_FIXED_R,
} spires_crossbar_model;
...
typedef struct {
	...
	double r_on;
	double r_off;
	spires_crossbar_model model;
} spires_crossbar_readout_config;

/* Run a trained reservoir with an online ngspice crossbar readout.
 * buffer must hold series_length * config->num_outputs values. */
spires_status spires_run_crossbar_readout_into(const spires_crossbar_readout_config *config,
...
double *spires_run_crossbar_readout(const spires_crossbar_readout_config *config,
					 spires_reservoir *reservoir,
					 const double *input_series,
					 size_t series_length);
#endif
 ...
 * Training
 * ---------------------------- */
spires_status spires_train_online(spires_reservoir *r, const double *target_vec,
				  double lr);
...
spires_status spires_train_rls(spires_reservoir *r, const double *input_series,
			       const double *target_series,
			       size_t series_length, double delta,
			       double lambda);
 ...
 * State access
 * ---------------------------- */
/* Returns a newly malloc'd copy of the current neuron state (length =
 * num_neurons). Caller must free().
 */
...
int spires_optimize(const spires_reservoir_config *base_config,
		    const struct spires_opt_budget *budgets, int num_budgets,
		    const struct spires_opt_score *score,
		    struct spires_opt_result *out, const double *input_series,
		    const double *target_series, size_t series_length);
